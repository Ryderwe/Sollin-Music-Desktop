...
const normalizeKuwoSearchCover = (path, type = 'album') => {
  if (!path) return null
  if (/^https?:/i.test(path)) return path
  const normalizedPath = String(path).replace(/^\/+/, '')
  const base = type === 'artist'
    ? 'https://img1.kuwo.cn/star/starheads/'
    : 'https://img1.kuwo.cn/star/albumcover/'
  ...
  handleResult(rawData) {
    ...
    for (let i = 0; i < rawData.length; i++) {
      ...
      result.push({
        ...
        lrc: null,
        img: normalizeKuwoSearchCover(info.web_albumpic_short) || normalizeKuwoSearchCover(info.web_artistpic_short, 'artist'),
        otherSource: null,
        types,
        _types,
        typeUrl: {},
