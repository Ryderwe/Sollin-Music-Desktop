...
const isValidSongId = (songId) => /^\d+$/.test(String(songId || '').trim())
...
export const decodeLyric = (lrc, tlrc, rlrc) => {
  if (!window.electronAPI?.decodeTxLyric) {
    return Promise.reject(new Error('QQ 歌词解码仅支持 Electron 桌面端'))
  }
  return window.electronAPI.decodeTxLyric({ lrc, tlrc, rlrc })
}
...
const fetchLyricBySongId = (songId) => {
...
const fetchLegacyLyricBySongmid = (songmid) => {
  ...
  return requestObj.promise.then(({ body }) => {
    ...
    return {
      lyric: decodeName(decodeBase64Text(body.lyric)),
      tlyric: decodeName(decodeBase64Text(body.trans)),
      rlyric: decodeName(decodeBase64Text(body.roma)),
      lxlyric: '',
    }
  })
}
...
export default {
  successCode: 0,
  async getSongId({ songId, songmid }, forceRefresh = false) {
    const normalizedSongId = String(songId || '').trim()
    if (!forceRefresh && isValidSongId(normalizedSongId)) return normalizedSongId
    if (!songmid) return normalizedSongId

    if (forceRefresh) songIdMap.delete(songmid)
    if (!forceRefresh && songIdMap.has(songmid)) return songIdMap.get(songmid)
    if (!forceRefresh && promises.has(songmid)) return (await promises.get(songmid)).songId

    const promise = getMusicInfo(songmid)
    promises.set(songmid, promise)
    try {
      const info = await promise
      const nextSongId = String(info?.songId || '').trim()
      if (isValidSongId(nextSongId)) songIdMap.set(songmid, nextSongId)
      return nextSongId
    } finally {
  ...
  async loadLyric(mInfo, { refreshSongId = false, allowLegacyFallback = true } = {}) {
    const songId = await this.getSongId(mInfo, refreshSongId)
    if (isValidSongId(songId)) {
      const body = await fetchLyricBySongId(songId)
      const lyricData = body?.req?.data
      if (body?.code == this.successCode && body?.req?.code == this.successCode && lyricData?.lyric) {
        return this.parseLyric(lyricData.lyric, lyricData.trans, lyricData.roma)
      }
    }

    if (!refreshSongId && mInfo?.songmid) {
      return this.loadLyric(mInfo, { refreshSongId: true, allowLegacyFallback })
    }

    if (allowLegacyFallback && mInfo?.songmid) {
      return fetchLegacyLyricBySongmid(mInfo.songmid)
    }
